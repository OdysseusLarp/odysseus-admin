<template>
  <span
    >{{ pressurePercent }} ({{ curveName }} scaling, raw =
    {{ Number(pressure).toFixed(3) }})</span
  >
</template>
  ...
  computed: {
    pressureRamp() {
      ...
      if (typeof pressure === "number")
        return { t0: 0, p0: pressure, t1: 0, p1: pressure };
  ...
  methods: {
    updatePressure() {
      ...
      if (now < ramp.t0) {
        ...
        this.pressureUpdateTimer = setTimeout(
          () => this.updatePressure(),
          Math.max(ramp.t0 - now, 100),
        );
      } else if (now >= ramp.t1) {
      ...
      this.$emit("pressureChange", {
        unscaled: this.pressure,
        scaled: this.scaledPressure,
      });
    },
  },
};
